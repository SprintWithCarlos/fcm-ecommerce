...
describe("Mobile", () => {
  it("passes", () => {
    ...
    cy.get('[data-testid="openCartDesktop"]').click();
    cy.get(
      '.navbar > .right > [data-testid="cartModal"] > .column > :nth-child(1) > img'
    ...
    cy.get(".thumb-strip ").should(($div) => {
      ...
      Array(buttons).forEach(($button) => {
        ...
        Array(images).forEach((image, i) => {
          expect(image).to.have.attr(
            "src",
            `/image-product-${i + 1}-thumbnail.jpg`
          );
        });
      });
    });
    [...Array(4)].forEach((item, i) => {
      cy.get('[data-testid="img"]').should(
        "have.attr",
        "src",
    ...
    [...Array(4)].forEach((item, i) => {
      ...
      if (i < 3) {
        cy.get(
          '.lightbox__wrapper > .thumb-strip > :nth-child(1) > [data-testid="thumbnail"]'
        );
        cy.get(
          `.lightbox__wrapper > .thumb-strip > :nth-child(${
            i + 2
    ...
    [...Array(4)].forEach((item) => {
      cy.get(
        '.lightbox__wrapper > .affordances > [data-testid="next"]'
      ).click();
      if (item < 4) {
        return cy
    ...
    [...Array(4)].forEach((item) => {
      cy.get(
        '.lightbox__wrapper > .affordances > [data-testid="back"]'
      ).click();
      if (item < 4) {
        return cy
          .get('[data-testid="image"]')
